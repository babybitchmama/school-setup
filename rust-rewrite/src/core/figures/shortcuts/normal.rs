...
fn compute_modifier_style(styles: &StylesConfig, pressed: &[String]) -> ComputedStyle {
    let held: HashSet<&str> = pressed.iter().map(String::as_str).collect();
    let settings = &styles.settings;

    let stroke_keys: HashSet<&str> = styles.modifiers.strokes.iter().map(|m| m.key.as_str()).collect();
    let dash_keys: HashSet<&str> = styles.modifiers.dashes.iter().map(|m| m.key.as_str()).collect();
    let arrow_keys: HashSet<&str> = styles.modifiers.arrows.iter().map(|m| m.key.as_str()).collect();

    let mut style = Style::new();
    style.insert("stroke-opacity".to_string(), "1".to_string());

    let any_stroke_related =
        !stroke_keys.is_disjoint(&held) || !dash_keys.is_disjoint(&held) || !arrow_keys.is_disjoint(&held);
    ...
    if has_arrow_end {
        ...
        style.insert("marker-end".to_string(), format!("url(#marker-arrow-{})", width));
    }
    if has_arrow_start {
        marker_width = Some(width);
        style.insert("marker-start".to_string(), format!("url(#marker-arrow-{})", width));
    }
        ...
        if held.contains(m.key.as_str()) {
            if m.description.to_lowercase().contains("dot") {
                style.insert(
                    ...
                    format!("{},{}", 3.0 * settings.pt_multiplier, 3.0 * settings.pt_multiplier),
                );
            } else {
                style.insert(
                    "stroke-dasharray".to_string(),
    ...
    for m in &styles.modifiers.fills {
        if held.contains(m.key.as_str()) {
            ...
            style.entry("fill-opacity".to_string()).or_insert("1".to_string());
        }
    }
    if has_fill {
        style.insert("marker-end".to_string(), "none".to_string());
    ...
    ComputedStyle { attributes: style, marker_width }
}
